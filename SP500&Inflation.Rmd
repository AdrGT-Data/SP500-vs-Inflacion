...
En Ingeniería de Datos me enseñaron a solo eliminar datos en casos muy concretos y como última opción. Sin embargo, este es un pequeño proyecto de análisis, por ello crearemos un nuevo dataset que una estos dos usando la columna `month`como clave. Esto permitirá crear un dataset común, que empiece cuando ambas series tienen datos y acabe cuando ambas terminan.
...
In Data Engineering teach me only eliminate data if there is not other option. However, this is a little proyect of analysis 
...
ES: El retorno mensual suele ser inestable en muchas inversiones. Usualemente lo que interesa a la hora de analizar inversiones es el retorno anual.
...
ES: Calcularemos estadísticos útiles para posibles inferencias futuras y para poder ver acerca de la naturaleza de los datos.
...
mezcla |> 
  ...
  pivot_longer(-month, names_to = "serie", values_to = "norm") |>
  ggplot(aes(month, norm, color = serie)) +
  geom_line(size = 1) +
  labs(
    title = "Evolución normalizada S&P 500 vs CPI",
    subtitle = "Índice base 100 en la primera observación",
    x = NULL, y = "Índice"
  ) +
  theme_minimal()
```
